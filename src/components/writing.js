import React from "react";

export default function Writing() {
  return (
    <React.Fragment>
      <>
        <section id="writing" className="py-20 mb-48 bg-yellow-500">
          <div className="container mx-auto px-4">
            <div className="flex justify-center mb-6">
              <div className="bg-yellow-100 py-1 px-2 font-bold inline-block">
                Writing
              </div>
            </div>
            <h1 className="font-heading text-center text-4xl lg:text-5xl font-bold max-w-2xl lg:max-w-5xl mx-auto mb-4 uppercase tracking-widest">
              Voice &amp; tone
            </h1>
            <p className="text-gray-600 text-center max-w-xl mx-auto mb-6">
              How we communicate to our audience tells our customers everything
              they need to know about us. Do we pay attention to the details? Do
              we get things right? Are we careful about how we and our products
              are represented? Any piece of writing or asset answers these
              questions immediately.
            </p>
            <div className="flex flex-wrap justify-center gap-4 mb-20"></div>
            <div className="flex flex-wrap" style={{ marginBottom: "-200px" }}>
              <div className="w-full lg:w-1/3">
                <div className="bg-white py-20 px-8 lg:mr-3 shadow-sm">
                  <span className="mb-6 flex justify-center"></span>
                  <h2 className="text-xl text-center mb-4 font-heading uppercase text-orange-500 font-bold tracking-widest">
                    Voice qualities
                  </h2>
                  <p className="text-gray-500 max-w-xs mx-auto text-center">
                    We're nice, patient, friendly, cheerful, down-to-earth, and
                    most of all, ourselves, because who else would we be? That's
                    what makes Harvey's, Harvey's.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/3">
                <div className="bg-white py-20 px-8 lg:mx-2 shadow-sm">
                  <span className="mb-6 flex justify-center"></span>
                  <h2 className="text-xl text-center mb-4 text-orange-500 font-heading tracking-widest font-bold uppercase">
                    Tone
                  </h2>
                  <p className="text-gray-500 max-w-xs mx-auto text-center">
                    We're honest, humble; we never talk down or act short. We
                    get to the point and speak plainly but powerfully, although
                    we're never dramatic.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/3">
                <div className="bg-white py-20 px-8 lg:ml-3 shadow-sm">
                  <span className="mb-6 flex justify-center"></span>
                  <h2 className="text-xl text-center mb-4 font-heading text-orange-500 tracking-widest uppercase font-bold">
                    Who we serve
                  </h2>
                  <p className="text-gray-500 max-w-xs mx-auto text-center">
                    Our customers are producers with 1000 head or more, down to
                    hobbyists with a single goat named Chuck. We serve serious
                    hunters as well as someone outfitting their first youth
                    hunt.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="pb-20">
          <div className="container mx-auto px-4">
            <div className="bg-yellow-100 px-2 py-1 font-bold inline-block mb-6">
              Tone
            </div>
            <div className="flex flex-wrap items-center -m-4 mb-12">
              <div className="w-full lg:w-1/2 p-4">
                <h1 className="font-heading font-bold text-4xl lg:text-5xl max-w-md lg:max-w-xl uppercase tracking-widest">
                  How we communicate
                </h1>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <p className="text-gray-600 max-w-md">
                  Just as important as the words we write are the way we say
                  them. Harvey's hasn't had a unified tone across sub-brands
                  before, but we believe coordinating around a central voice
                  will amplify the brand going forward.
                </p>
              </div>
            </div>
            <div className="flex flex-row flex-wrap">
              <div className="bg-gray-50 p-12 w-full md:w-1/2">
                <div className="flex flex-wrap items-center -m-4">
                  <div className="w-full p-4">
                    <h2 className="font-heading text-2xl font-bold uppercase tracking-widest">
                      Harvey's is
                    </h2>
                  </div>
                  <div className="w-full p-4">
                    <ul>
                      <li className="flex gap-2 items-center mb-3">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            fill="none"
                          >
                            <path
                              d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                            <path
                              d="M8 11.5L11 14.5L17 8.5"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Helpful</p>
                      </li>
                      <li className="flex gap-2 items-center mb-3">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            fill="none"
                          >
                            <path
                              d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                            <path
                              d="M8 11.5L11 14.5L17 8.5"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Honest</p>
                      </li>
                      <li className="flex gap-2 items-center mb-3">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            fill="none"
                          >
                            <path
                              d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                            <path
                              d="M8 11.5L11 14.5L17 8.5"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Positive</p>
                      </li>
                      <li className="flex gap-2 items-center">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            fill="none"
                          >
                            <path
                              d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                            <path
                              d="M8 11.5L11 14.5L17 8.5"
                              stroke="#006251"
                              strokeWidth={2}
                              strokeLinecap="round"
                              strokeLinejoin="round"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Dependable</p>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
              <div className="bg-gray-50 p-12 w-full md:w-1/2">
                <div className="flex flex-wrap items-center -m-4">
                  <div className="w-full p-4">
                    <h2 className="font-heading text-2xl font-bold uppercase tracking-widest">
                      Harvey's is not
                    </h2>
                  </div>
                  <div className="w-full p-4">
                    <ul>
                      <li className="flex gap-2 items-center mb-3">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            strokeWidth={2}
                            stroke="currentColor"
                            aria-hidden="true"
                            className
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"
                              stroke="#c02617"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Aggressive</p>
                      </li>
                      <li className="flex gap-2 items-center mb-3">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            strokeWidth={2}
                            stroke="currentColor"
                            aria-hidden="true"
                            className
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"
                              stroke="#c02617"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Pretentious</p>
                      </li>
                      <li className="flex gap-2 items-center mb-3">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            strokeWidth={2}
                            stroke="currentColor"
                            aria-hidden="true"
                            className
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"
                              stroke="#c02617"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Negative</p>
                      </li>
                      <li className="flex gap-2 items-center">
                        <span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            width={24}
                            height={24}
                            viewBox="0 0 24 24"
                            strokeWidth={2}
                            stroke="currentColor"
                            aria-hidden="true"
                            className
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"
                              stroke="#c02617"
                            />
                          </svg>
                        </span>
                        <p className="text-gray-500">Rigidly corporate</p>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="py-20">
          <div className="container mx-auto px-4">
            <div className="flex flex-wrap items-center -m-4">
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-yellow-500 px-2 py-1 mb-6 text-xs font-bold inline-block">
                  Email campaigns
                </div>
                <h1 className="font-heading text-4xl font-bold mb-6 max-w-md uppercase tracking-widest">
                  Email guidelines
                </h1>
                <p className="text-gray-500 max-w-md mb-6">
                  Our email campaigns are a key touchpoint with
                  customers—offering them a window into what’s new, what’s in
                  store, and what’s worth checking out online. These emails are
                  crafted to inspire, inform, and drive action.
                </p>
                <p className="text-gray-500 max-w-md mb-6">
                  We are simple and direct. Not
                  <span className="font-bold">overly wordy</span> . We let the
                  products and images speak for themselves.
                </p>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-gray-50 p-12">
                  <div className="flex gap-6 mb-8">
                    <div>
                      <h2 className="text-xl font-bold mb-3">
                        Product Highlights
                      </h2>
                      <p className="text-gray-500">
                        Utilize brand assets for product photography and
                        lifestyle images.
                      </p>
                      <ul className="list-disc">
                        <li className>Include brand and product name</li>
                        <li className>
                          Feature individual product shots with short,
                          compelling captions
                        </li>
                        <li className>
                          Include key details (e.g. fabric, fit) in 1–2 short
                          sentences
                        </li>
                        <li className>
                          If applicable, always include a clear link to shop the
                          item on our website
                        </li>
                        <li className>
                          Mix and match products from brands we carry (like
                          Carhartt, Kuhl, prAna, and Patagonia) to tell seasonal
                          or lifestyle stories
                        </li>
                        <li className>
                          Incorporate ‘top picks, ’ ‘customer favorites,’ or
                          styled sets that pair well together
                        </li>
                      </ul>
                    </div>
                  </div>
                  <div className="flex gap-6 mb-8">
                    <div>
                      <h2 className="text-xl font-bold mb-3">
                        Sales &amp; Events
                      </h2>
                      <p className="text-gray-500">
                        Keep sales messaging clean and focused—highlight the
                        value, not just the discount. Use bold imagery to draw
                        customers in and short captions to create urgency. For
                        events, flyers may be used as an email template.
                      </p>
                    </div>
                  </div>
                  <div className="flex gap-6"> </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <section className="py-10 ">
          <div className="container mx-auto px-4">
            <h2 className="font-heading uppercase tracking-widest text-orange-500 font-bold text-center py-10 text-4xl">
              Sample messaging
            </h2>
            <div className="flex -m-4 flex-wrap">
              <div className="w-full md:w-1/2 p-4 mb-12">
                <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                  HARVEY'S RETAIL SOCIAL
                </p>
                <h2 className="h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                  FACEBOOK POST
                </h2>
                <img
                  alt="facebook post"
                  src="https://static.shuffle.dev/uploads/files/ee/ee137d2a3df0a9e3dc5d8722b3ba155636636edc/Screenshot-2025-03-19-at-7-53-06-PM.jpg"
                />
                <p className="text-gray-600 mt-4 mb-4">
                  We prefer to show customers or use professional photos from
                  the asset library but when none are available but a new
                  product is in stock, close up or displays from an iPhone are
                  acceptable
                </p>
              </div>
              <div className="w-full md:w-1/2 p-4 mb-12 lg:mb-0">
                <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                  HARVEY'S retail social
                </p>
                <h2 className="h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                  instagram post
                </h2>
                <img
                  alt="Instagram post"
                  src="https://static.shuffle.dev/uploads/files/ee/ee137d2a3df0a9e3dc5d8722b3ba155636636edc/Screenshot-2025-03-19-at-7-52-28-PM.jpg"
                />
                <p className="text-gray-600 mt-4 mb-4">
                  When promoting new products, showing actual customers enjoying
                  their Harvey's gear always does the best on socials. Make sure
                  to get permission from parents or guardians before posting
                  publically.
                </p>
              </div>
              <div className="w-full md:w-1/2 p-4">
                <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                  HARVEY'S
                </p>
                <h2 className="h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                  Press bio
                </h2>
                <p className="text-gray-600 mb-4 max-w-xs">
                  Founded in 1955 and headquartered in Carson City, Michigan,
                  Harvey's is a diversified agricultural leader specializing in
                  premium seed solutions, customized livestock nutrition, agile
                  commodities logistics, quality retail products, and seasonal
                  lawn and garden offerings. Harvey's operates through five
                  collaborative divisions—Seed, Feed, Commodities, Retail, and
                  the Garden Center.
                </p>
                <a
                  className="inline-block text-green-500 hover:text-green-600 transition duration-200 font-bold"
                  href="#"
                >
                  copy
                </a>
              </div>
            </div>
          </div>
        </section>
        <section id="architecture" className="py-20 bg-green-700 ">
          <div className="container mx-auto px-4">
            <div className="flex justify-center mb-6">
              <div className="bg-green-400 inline-block px-2 py-1 text-white font-bold">
                Architecture
              </div>
            </div>
            <h1 className="font-heading text-4xl lg:text-5xl text-center mx-auto mb-16 text-white font-bold max-w-lg lg:max-w-3xl uppercase tracking-widest">
              BRAND HIERARCHY
            </h1>
            <div className="flex flex-wrap -m-4">
              <div className="w-full p-4">
                <div className="bg-white p-12 flex flex-col items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    id="Layer_2"
                    data-name="Layer 2"
                    viewBox="0 0 340.25 215.77"
                    className="h-40"
                  >
                    <defs />
                    <g id="Layer_1-2" data-name="Layer 1">
                      <path
                        className="cls-1"
                        d="M170.13,5.82c15.97,0,28.74,5.62,37.95,16.71,9.92,11.94,15.38,29.55,15.38,49.6s-5.46,37.66-15.38,49.6c-9.21,11.09-21.98,16.71-37.95,16.71s-28.74-5.62-37.95-16.71c-9.92-11.94-15.38-29.55-15.38-49.6s5.46-37.66,15.38-49.6c9.21-11.09,21.98-16.71,37.95-16.71M170.13,0c-17.79,0-32.06,6.33-42.43,18.81-10.78,12.98-16.72,31.92-16.72,53.31s5.94,40.33,16.72,53.31c10.37,12.48,24.64,18.81,42.43,18.81s32.06-6.33,42.43-18.81c10.78-12.98,16.72-31.92,16.72-53.31s-5.94-40.33-16.72-53.31C202.19,6.33,187.91,0,170.13,0h0ZM205.11,45.15l-2.07,4.18-.04,53.35,2.11,4.48v4.36h-16.52s-7.99-8.03-7.99-8.03v-24.5c-6.53-.95-13.33-1.35-19.62.63-.26.08-1.04.29-1.22.4-.03.02-.06.03-.07.07l-.05,31.43h-16.47c-.1-.22-.31-.42-.48-.59-2.31-2.23-4.59-4.53-6.83-6.83-.09-.1-.68-.59-.69-.65v-4.16s2.04-4.44,2.04-4.44l.02-53.08c-.58-1.5-1.37-2.94-2.06-4.4v-4.62h16.52c2.01,2.13,4.17,4.18,6.24,6.27.48.48,1.03,1.13,1.52,1.59.03.03.24.12.24.13v17.67c7-1.1,14.24-.33,20.92,1.92v-27.58s16.52,0,16.52,0c2.47,2.55,5.02,5.07,7.54,7.59.13.13.29.32.46.4v4.41ZM183.37,64.25c-2.19-.69-4.33-1.55-6.55-2.14-7.42-1.98-15.26-1.85-22.61.35-1.79.54-3.53,1.24-5.32,1.8v-28.66s-.07-.07-.07-.07h-10.85v1.03c0,.15.03.31.1.45.65,1.36,1.37,2.69,1.91,4.09v54.21c-.55,1.5-1.3,2.94-1.95,4.4-.04.09-.06.2-.06.3h0c0,.4.33.73.73.73h10.12s.07-.07.07-.07v-30.62c1.71-.54,3.38-1.15,5.11-1.61,9.68-2.57,20.06-2,29.36,1.69v30.62s10.97,0,10.97,0v-.93c-.71-1.51-1.51-3-2.11-4.55l.04-54.55,2.07-4.2v-.98s-10.97,0-10.97,0v28.73ZM0,171.73h9.01v18.43h19.6v-18.43h9.01v44.04h-9.01v-18.31H9.01v18.31H0v-44.04ZM64.42,171.73h7.03c.35,0,.67.22.8.55l16.29,42.32c.22.56-.2,1.17-.8,1.17h-7.3c-.35,0-.67-.22-.8-.55l-3.35-8.66c-.13-.33-.45-.55-.8-.55h-15.12c-.35,0-.67.22-.8.55l-3.35,8.66c-.13.33-.45.55-.8.55h-7.3c-.6,0-1.02-.6-.8-1.17l16.29-42.32c.13-.33.45-.55.8-.55ZM72.87,198.97c.6,0,1.01-.59.81-1.15l-5.73-15.58h-.13l-5.55,15.59c-.2.56.22,1.15.81,1.15h9.81ZM136.33,201.52v13.39c0,.47-.38.86-.86.86h-7.29c-.47,0-.86-.38-.86-.86v-11.45c0-.22-.08-.43-.23-.59l-3.49-3.7c-.16-.17-.39-.27-.62-.27h-13.21c-.47,0-.86.38-.86.86v15.15c0,.47-.38.86-.86.86h-7.29c-.47,0-.86-.38-.86-.86v-42.32c0-.47.38-.86.86-.86h27.87c.22,0,.43.09.59.24l6.57,6.3c.17.16.26.39.26.62v12.61c0,.23-.09.46-.26.62l-3.62,3.49c-.34.33-.35.87-.02,1.21l3.94,4.1c.15.16.24.37.24.59ZM109.77,191.86h14.76c.22,0,.43-.08.59-.24l1.8-1.71c.17-.16.27-.39.27-.62v-7.82c0-.24-.1-.46-.27-.62l-1.8-1.71c-.16-.15-.37-.24-.59-.24h-14.76c-.47,0-.86.38-.86.86v11.24c0,.47.38.86.86.86ZM185.17,171.73h-6.77c-.23,0-.44.15-.52.37l-11.99,31.14c-.1.27-.48.27-.59,0l-11.69-31.16c-.08-.21-.28-.35-.51-.35h-6.76c-.38,0-.64.38-.51.74l16.01,42.75c.13.34.45.56.8.56h6.33c.13,0,.25-.08.29-.2l16.42-43.12c.12-.35-.14-.72-.52-.72ZM197.98,171.73h31.79c.47,0,.86.38.86.86v5.52c0,.47-.38.86-.86.86h-22.79c-.47,0-.86.38-.86.86v9.42c0,.47.38.86.86.86h20.87c.47,0,.86.38.86.86v5.39c0,.47-.38.86-.86.86h-20.87c-.47,0-.86.38-.86.86v9.61c0,.47.38.86.86.86h22.79c.47,0,.86.38.86.86v5.52c0,.47-.38.86-.86.86h-31.79c-.47,0-.86-.38-.86-.86v-42.32c0-.47.38-.86.86-.86ZM256.04,197.57l-15.52-24.52c-.36-.57.05-1.32.72-1.32h7.83c.3,0,.58.16.73.41l10.81,17.65h.13l10.81-17.65c.16-.25.43-.41.73-.41h7.83c.68,0,1.09.75.72,1.32l-15.52,24.52c-.09.14-.13.3-.13.46v16.89c0,.47-.38.86-.86.86h-7.29c-.47,0-.86-.38-.86-.86v-16.89c0-.16-.05-.32-.13-.46ZM287.97,178.85v-6.27c0-.47.38-.86.86-.86h7.29c.47,0,.86.38.86.86v6.27s0,.07,0,.1l-.97,8.38c-.05.43-.42.76-.85.76h-5.36c-.44,0-.8-.33-.85-.76l-.97-8.38s0-.07,0-.1ZM304.36,208.61v-5.07c0-.47.38-.86.86-.86h7.16c.47,0,.86.38.86.86v2.49c0,.24.1.46.27.62l1.67,1.59c.16.15.37.24.59.24h13.02c.22,0,.43-.08.59-.24l1.74-1.65c.17-.16.27-.39.27-.62v-6.56c0-.24-.1-.46-.27-.62l-1.67-1.59c-.16-.15-.37-.24-.59-.24h-16.86c-.22,0-.43-.08-.59-.24l-6.64-6.31c-.17-.16-.27-.39-.27-.62v-10.9c0-.24.1-.46.27-.62l6.64-6.31c.16-.15.37-.24.59-.24h20.24c.22,0,.43.08.59.24l6.64,6.31c.17.16.27.39.27.62v5.13c0,.47-.38.86-.86.86h-7.16c-.47,0-.86-.38-.86-.86v-2.55c0-.24-.1-.46-.27-.62l-1.67-1.59c-.16-.15-.37-.24-.59-.24h-12.43c-.22,0-.43.08-.59.24l-1.67,1.59c-.17.16-.27.39-.27.62v5.74c0,.24.1.46.27.62l1.67,1.59c.16.15.37.24.59.24h16.86c.22,0,.43.08.59.24l6.64,6.31c.17.16.27.39.27.62v11.66c0,.24-.1.46-.27.62l-6.77,6.43c-.16.15-.37.24-.59.24h-20.77c-.22,0-.43-.08-.59-.24l-6.64-6.31c-.17-.16-.27-.39-.27-.62Z"
                      />
                    </g>
                  </svg>
                  <h6 className="text-gray-600 max-w-md text-xl font-bold mb-8 mt-4 h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                    Parent brand
                  </h6>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Tagline
                  </p>
                  <p className="text-gray-600 max-w-md mb-1">
                    Generations of Service, Built on Trust.
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Driven by quality, defined by service
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Brand history
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Harvey Milling Co., Inc. was founded in 1964 and began the
                    modern conception of the brand. Harvey's expanded from
                    traditional retail and feed mill with a seasonal farm and
                    garden center as well as a state-of-the-art chemical,
                    fertilizer, and seed business east of town that was sold in
                    2011.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence description
                  </p>
                  <p className="text-gray-600 max-w-md">
                    Harvey's is Carson City's landmark agricultural brand,
                    offering premium seed, custom feed solutions, flexible
                    commodities logistics, seasonal garden essentials, and
                    quality retail products—all backed by unmatched service and
                    deep-rooted local expertise.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-white p-12 flex flex-col items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    id="Layer_2"
                    data-name="Layer 2"
                    viewBox="0 0 326.51 251.99"
                    className="h-40"
                  >
                    <defs />
                    <g id="Layer_1-2" data-name="Layer 1">
                      <path
                        className="cls-1"
                        d="M0,165.74h8.64v17.69h18.81v-17.69h8.64v42.26h-8.64v-17.57H8.64v17.57H0v-42.26ZM61.82,165.74h6.75c.34,0,.65.21.77.53l15.63,40.61c.21.54-.19,1.12-.77,1.12h-7.01c-.34,0-.64-.21-.77-.53l-3.22-8.31c-.12-.32-.43-.53-.77-.53h-14.51c-.34,0-.64.21-.77.53l-3.22,8.31c-.12.32-.43.53-.77.53h-7.01c-.58,0-.98-.58-.77-1.12l15.63-40.61c.12-.32.43-.53.77-.53ZM69.93,191.88c.57,0,.97-.57.77-1.11l-5.5-14.95h-.13l-5.33,14.96c-.19.54.21,1.1.78,1.1h9.41ZM130.83,194.33v12.85c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-10.98c0-.21-.08-.41-.22-.56l-3.35-3.55c-.16-.16-.37-.26-.6-.26h-12.68c-.45,0-.82.37-.82.82v14.53c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82h26.74c.21,0,.42.08.57.23l6.31,6.05c.16.16.25.37.25.59v12.1c0,.22-.09.44-.25.59l-3.48,3.35c-.33.31-.34.83-.02,1.16l3.78,3.94c.15.15.23.36.23.57ZM105.34,185.06h14.16c.21,0,.41-.08.57-.23l1.73-1.64c.16-.16.26-.37.26-.6v-7.5c0-.23-.09-.44-.26-.6l-1.73-1.64c-.15-.15-.36-.23-.57-.23h-14.16c-.45,0-.82.37-.82.82v10.79c0,.45.37.82.82.82ZM177.69,165.74h-6.5c-.22,0-.42.14-.49.35l-11.51,29.89c-.1.26-.47.26-.56,0l-11.22-29.9c-.08-.2-.27-.34-.49-.34h-6.49c-.36,0-.62.36-.49.71l15.36,41.02c.12.32.43.53.77.53h6.07c.12,0,.24-.08.28-.19l15.76-41.38c.12-.34-.14-.69-.49-.69ZM189.98,165.74h30.51c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-21.87c-.45,0-.82.37-.82.82v9.04c0,.45.37.82.82.82h20.02c.45,0,.82.37.82.82v5.18c0,.45-.37.82-.82.82h-20.02c-.45,0-.82.37-.82.82v9.22c0,.45.37.82.82.82h21.87c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-30.51c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82ZM245.7,190.53l-14.9-23.53c-.35-.55.05-1.26.7-1.26h7.51c.29,0,.55.15.7.39l10.37,16.93h.13l10.37-16.93c.15-.24.42-.39.7-.39h7.51c.65,0,1.04.72.7,1.26l-14.9,23.53c-.08.13-.13.28-.13.44v16.2c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-16.2c0-.16-.04-.31-.13-.44ZM276.34,172.58v-6.01c0-.45.37-.82.82-.82h7c.45,0,.82.37.82.82v6.01s0,.06,0,.09l-.93,8.04c-.05.42-.4.73-.82.73h-5.14c-.42,0-.77-.31-.82-.73l-.93-8.04s0-.06,0-.09ZM292.06,201.13v-4.86c0-.45.37-.82.82-.82h6.87c.45,0,.82.37.82.82v2.39c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h12.5c.21,0,.41-.08.57-.23l1.67-1.58c.16-.16.26-.37.26-.6v-6.3c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-16.18c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6v-10.46c0-.23.09-.44.26-.6l6.37-6.05c.15-.15.36-.23.57-.23h19.42c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v4.92c0,.45-.37.82-.82.82h-6.87c-.45,0-.82-.37-.82-.82v-2.45c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-11.93c-.21,0-.41.08-.57.23l-1.6,1.52c-.16.16-.26.37-.26.6v5.51c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h16.18c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v11.19c0,.23-.09.44-.26.6l-6.5,6.17c-.15.15-.36.23-.57.23h-19.93c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6ZM163.25,5.54c15.21,0,27.36,5.35,36.13,15.91,9.44,11.37,14.64,28.13,14.64,47.21s-5.2,35.85-14.64,47.21c-8.77,10.56-20.92,15.91-36.13,15.91s-27.36-5.35-36.13-15.91c-9.44-11.37-14.64-28.13-14.64-47.21s5.2-35.85,14.64-47.21c8.77-10.56,20.92-15.91,36.13-15.91M163.25,0c-16.93,0-30.52,6.03-40.39,17.91-10.26,12.36-15.92,30.38-15.92,50.75s5.65,38.39,15.92,50.75c9.87,11.89,23.46,17.91,40.39,17.91s30.52-6.03,40.39-17.91c10.26-12.36,15.92-30.38,15.92-50.75s-5.65-38.39-15.92-50.75C193.78,6.03,180.19,0,163.25,0h0ZM196.56,42.98l-1.97,3.98-.04,50.79,2.01,4.26v4.15h-15.73s-7.61-7.65-7.61-7.65v-23.32s0,0,0,0c-6.21-.91-12.69-1.29-18.68.6-.25.08-.99.28-1.17.38-.03.02-.06.03-.07.07l-.05,29.92h-15.68c-.09-.21-.29-.4-.46-.56-2.2-2.13-4.37-4.31-6.5-6.5-.09-.09-.65-.56-.65-.62v-3.96s1.94-4.23,1.94-4.23l.02-50.53c-.56-1.43-1.3-2.8-1.96-4.19v-4.4h15.73c1.91,2.03,3.97,3.98,5.94,5.97.45.46.98,1.08,1.44,1.52.03.03.23.11.23.12v16.82c6.66-1.04,13.56-.32,19.92,1.82v-26.26s15.73,0,15.73,0c2.35,2.43,4.78,4.83,7.18,7.23.13.13.28.3.44.38v4.19ZM175.86,61.17s0,0,0,0c-2.08-.66-4.12-1.47-6.23-2.04-7.06-1.89-14.53-1.76-21.53.33-1.71.51-3.36,1.18-5.06,1.71v-27.28s-.07-.07-.07-.07h-10.32s0,.98,0,.98c0,.15.03.29.1.43.62,1.29,1.3,2.56,1.82,3.9v51.61c-.53,1.43-1.24,2.8-1.86,4.19-.04.09-.06.19-.06.28h0c0,.38.31.69.69.69h9.63s.07-.07.07-.07v-29.15c1.62-.51,3.22-1.1,4.87-1.54,9.22-2.45,19.1-1.9,27.95,1.61v29.15s10.44,0,10.44,0v-.89c-.68-1.44-1.43-2.85-2.01-4.33l.04-51.93,1.97-4v-.93s-10.44,0-10.44,0v27.35ZM42.67,248.35v-14.87l3.64-3.64h10.82l3.58,3.57v3.38h-4.3v-1.8l-1.49-1.49h-6.33l-1.61,1.61v11.58l1.61,1.61h6.33l1.49-1.49v-1.8h4.3v3.38l-3.58,3.57h-10.82l-3.64-3.64ZM66.56,248.35v-14.87l3.64-3.64h11.39l3.64,3.64v14.87l-3.64,3.64h-11.39l-3.64-3.64ZM79.3,248.32l1.61-1.61v-11.58l-1.61-1.61h-6.83l-1.61,1.61v11.58l1.61,1.61h6.83ZM92.18,229.84h3.99l6.58,14.3h.06l6.61-14.3h3.99v22.14h-4.14v-13.48h-.06l-5.09,10.38h-2.66l-5.06-10.38h-.06v13.48h-4.14v-22.14ZM120.68,229.84h3.99l6.58,14.3h.06l6.61-14.3h3.99v22.14h-4.14v-13.48h-.06l-5.09,10.38h-2.66l-5.06-10.38h-.06v13.48h-4.14v-22.14ZM148.86,248.35v-14.87l3.64-3.64h11.39l3.64,3.64v14.87l-3.64,3.64h-11.39l-3.64-3.64ZM161.61,248.32l1.61-1.61v-11.58l-1.61-1.61h-6.83l-1.61,1.61v11.58l1.61,1.61h6.83ZM174.48,229.84h14.17l3.8,3.8v14.55l-3.8,3.8h-14.17v-22.14ZM186.54,248.32l1.61-1.61v-11.58l-1.61-1.61h-7.75v14.8h7.75ZM199.57,229.84h4.3v22.14h-4.3v-22.14ZM216.33,233.48h-6.61v-3.64h17.52v3.64h-6.61v18.5h-4.3v-18.5ZM233.1,229.84h4.3v22.14h-4.3v-22.14ZM244.83,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM266.69,248.57v-3.16h4.24v1.87l1.04,1.04h6.55l1.08-1.08v-3.67l-1.04-1.04h-8.38l-3.42-3.42v-5.85l3.42-3.42h10l3.42,3.42v3.2h-4.24v-1.9l-1.04-1.04h-6.26l-1.04,1.04v3.26l1.04,1.04h8.38l3.42,3.42v6.23l-3.48,3.48h-10.25l-3.42-3.42Z"
                      />
                    </g>
                  </svg>
                  <h6 className="text-gray-600 max-w-md text-xl font-bold mb-8 mt-4 h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                    Sub-brand
                  </h6>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Tagline
                  </p>
                  <p className="text-gray-600 max-w-md mb-1">
                    Flexible, reliable, feed solutions on time
                  </p>
                  <p className="text-gray-600 max-w-md mb-1">
                    Reliable Feed. Flexible Solutions. Always On Time
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Feed You Trust. Service You Count On
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Brand history
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Shipping to over 200 large dairies across 7 states with
                    annual deliveries topping 1 million tons, Harvey's
                    Commodities is a behemoth of agile logistics and
                    dependability.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence description
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Harvey's Commodities delivers flexible, reliable feed
                    solutions and expert contracting services, simplifying
                    nutrition management for dairy operations across the
                    Midwest.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence CTA
                  </p>
                  <p className="text-gray-600 max-w-md">
                    Simplify your dairy nutrition—partner with Harvey’s
                    Commodities for flexible feed solutions, expert service, and
                    logistics you can count on.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-white p-12 flex flex-col items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    id="Layer_2"
                    data-name="Layer 2"
                    viewBox="0 0 326.51 251.99"
                    className="h-40"
                  >
                    <defs />
                    <g id="Layer_1-2" data-name="Layer 1">
                      <path
                        className="cls-1"
                        d="M0,165.74h8.64v17.69h18.81v-17.69h8.64v42.26h-8.64v-17.57H8.64v17.57H0v-42.26ZM61.82,165.74h6.75c.34,0,.65.21.77.53l15.63,40.61c.21.54-.19,1.12-.77,1.12h-7.01c-.34,0-.64-.21-.77-.53l-3.22-8.31c-.12-.32-.43-.53-.77-.53h-14.51c-.34,0-.64.21-.77.53l-3.22,8.31c-.12.32-.43.53-.77.53h-7.01c-.58,0-.98-.58-.77-1.12l15.63-40.61c.12-.32.43-.53.77-.53ZM69.93,191.88c.57,0,.97-.57.77-1.11l-5.5-14.95h-.13l-5.33,14.96c-.19.54.21,1.1.78,1.1h9.41ZM130.83,194.33v12.85c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-10.98c0-.21-.08-.41-.22-.56l-3.35-3.55c-.16-.16-.37-.26-.6-.26h-12.68c-.45,0-.82.37-.82.82v14.53c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82h26.74c.21,0,.42.08.57.23l6.31,6.05c.16.16.25.37.25.59v12.1c0,.22-.09.44-.25.59l-3.48,3.35c-.33.31-.34.83-.02,1.16l3.78,3.94c.15.15.23.36.23.57ZM105.34,185.06h14.16c.21,0,.41-.08.57-.23l1.73-1.64c.16-.16.26-.37.26-.6v-7.5c0-.23-.09-.44-.26-.6l-1.73-1.64c-.15-.15-.36-.23-.57-.23h-14.16c-.45,0-.82.37-.82.82v10.79c0,.45.37.82.82.82ZM177.69,165.74h-6.5c-.22,0-.42.14-.49.35l-11.51,29.89c-.1.26-.47.26-.56,0l-11.22-29.9c-.08-.2-.27-.34-.49-.34h-6.49c-.36,0-.62.36-.49.71l15.36,41.02c.12.32.43.53.77.53h6.07c.12,0,.24-.08.28-.19l15.76-41.38c.12-.34-.14-.69-.49-.69ZM189.98,165.74h30.51c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-21.87c-.45,0-.82.37-.82.82v9.04c0,.45.37.82.82.82h20.02c.45,0,.82.37.82.82v5.18c0,.45-.37.82-.82.82h-20.02c-.45,0-.82.37-.82.82v9.22c0,.45.37.82.82.82h21.87c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-30.51c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82ZM245.7,190.53l-14.9-23.53c-.35-.55.05-1.26.7-1.26h7.51c.29,0,.55.15.7.39l10.37,16.93h.13l10.37-16.93c.15-.24.42-.39.7-.39h7.51c.65,0,1.04.72.7,1.26l-14.9,23.53c-.08.13-.13.28-.13.44v16.2c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-16.2c0-.16-.04-.31-.13-.44ZM276.34,172.58v-6.01c0-.45.37-.82.82-.82h7c.45,0,.82.37.82.82v6.01s0,.06,0,.09l-.93,8.04c-.05.42-.4.73-.82.73h-5.14c-.42,0-.77-.31-.82-.73l-.93-8.04s0-.06,0-.09ZM292.06,201.13v-4.86c0-.45.37-.82.82-.82h6.87c.45,0,.82.37.82.82v2.39c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h12.5c.21,0,.41-.08.57-.23l1.67-1.58c.16-.16.26-.37.26-.6v-6.3c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-16.18c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6v-10.46c0-.23.09-.44.26-.6l6.37-6.05c.15-.15.36-.23.57-.23h19.42c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v4.92c0,.45-.37.82-.82.82h-6.87c-.45,0-.82-.37-.82-.82v-2.45c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-11.93c-.21,0-.41.08-.57.23l-1.6,1.52c-.16.16-.26.37-.26.6v5.51c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h16.18c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v11.19c0,.23-.09.44-.26.6l-6.5,6.17c-.15.15-.36.23-.57.23h-19.93c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6ZM163.25,5.54c15.21,0,27.36,5.35,36.13,15.91,9.44,11.37,14.64,28.13,14.64,47.21s-5.2,35.85-14.64,47.21c-8.77,10.56-20.92,15.91-36.13,15.91s-27.36-5.35-36.13-15.91c-9.44-11.37-14.64-28.13-14.64-47.21s5.2-35.85,14.64-47.21c8.77-10.56,20.92-15.91,36.13-15.91M163.25,0c-16.93,0-30.52,6.03-40.39,17.91-10.26,12.36-15.92,30.38-15.92,50.75s5.65,38.39,15.92,50.75c9.87,11.89,23.46,17.91,40.39,17.91s30.52-6.03,40.39-17.91c10.26-12.36,15.92-30.38,15.92-50.75s-5.65-38.39-15.92-50.75C193.78,6.03,180.19,0,163.25,0h0ZM196.56,42.98l-1.97,3.98-.04,50.79,2.01,4.26v4.15h-15.73s-7.61-7.65-7.61-7.65v-23.32s0,0,0,0c-6.21-.91-12.69-1.29-18.68.6-.25.08-.99.28-1.17.38-.03.02-.06.03-.07.07l-.05,29.92h-15.68c-.09-.21-.29-.4-.46-.56-2.2-2.13-4.37-4.31-6.5-6.5-.09-.09-.65-.56-.65-.62v-3.96s1.94-4.23,1.94-4.23l.02-50.53c-.56-1.43-1.3-2.8-1.96-4.19v-4.4h15.73c1.91,2.03,3.97,3.98,5.94,5.97.45.46.98,1.08,1.44,1.52.03.03.23.11.23.12v16.82c6.66-1.04,13.56-.32,19.92,1.82v-26.26s15.73,0,15.73,0c2.35,2.43,4.78,4.83,7.18,7.23.13.13.28.3.44.38v4.19ZM175.86,61.17s0,0,0,0c-2.08-.66-4.12-1.47-6.23-2.04-7.06-1.89-14.53-1.76-21.53.33-1.71.51-3.36,1.18-5.06,1.71v-27.28s-.07-.07-.07-.07h-10.32s0,.98,0,.98c0,.15.03.29.1.43.62,1.29,1.3,2.56,1.82,3.9v51.61c-.53,1.43-1.24,2.8-1.86,4.19-.04.09-.06.19-.06.28h0c0,.38.31.69.69.69h9.63s.07-.07.07-.07v-29.15c1.62-.51,3.22-1.1,4.87-1.54,9.22-2.45,19.1-1.9,27.95,1.61v29.15s10.44,0,10.44,0v-.89c-.68-1.44-1.43-2.85-2.01-4.33l.04-51.93,1.97-4v-.93s-10.44,0-10.44,0v27.35ZM119.97,248.57v-3.16h4.24v1.87l1.04,1.04h6.55l1.08-1.08v-3.67l-1.04-1.04h-8.38l-3.42-3.42v-5.85l3.42-3.42h10l3.42,3.42v3.2h-4.24v-1.9l-1.04-1.04h-6.26l-1.04,1.04v3.26l1.04,1.04h8.38l3.42,3.42v6.23l-3.48,3.48h-10.25l-3.42-3.42ZM143.75,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM166.09,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM188.42,229.84h14.17l3.8,3.8v14.55l-3.8,3.8h-14.17v-22.14ZM200.47,248.32l1.61-1.61v-11.58l-1.61-1.61h-7.75v14.8h7.75Z"
                      />
                    </g>
                  </svg>
                  <h6 className="text-gray-600 max-w-md text-xl font-bold mb-8 mt-4 h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                    Sub-brand
                  </h6>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Tagline
                  </p>
                  <p className="text-gray-600 max-w-md mb-1">
                    Premium seed, season after season
                  </p>
                  <p className="text-gray-600 max-w-md mb-1">
                    Premium Seed. Proven Every Season
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Plant with Confidence. Harvest with Trust
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Brand history
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    One could be forgiven thinking Harvey's Seed is another
                    iteration of Harvey's Agricultural Solutions; except,
                    Harvey's Seed is a much leaner, more focused endeavor.
                    Harvey's Seed is the nimble predecessor with a focus on
                    speciality seeds, non-GMO, cover crops, high management, and
                    traditional row.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence description
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Harvey's Seed supports Michigan growers with premium seed,
                    experienced guidance, and dedicated service—helping farms
                    perform at their best, season after season.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence CTA
                  </p>
                  <p className="text-gray-600 max-w-md">
                    At Harvey’s Seed, we’re proud to partner with some of the
                    finest growers in the world, combining premium seed with
                    exceptional service to keep your farm thriving no matter the
                    circumstance.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-white p-12 flex flex-col items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    id="Layer_2"
                    data-name="Layer 2"
                    viewBox="0 0 326.51 251.99"
                    className="h-40"
                  >
                    <defs />
                    <g id="Layer_1-2" data-name="Layer 1">
                      <path
                        className="cls-1"
                        d="M0,165.74h8.64v17.69h18.81v-17.69h8.64v42.26h-8.64v-17.57H8.64v17.57H0v-42.26ZM61.82,165.74h6.75c.34,0,.65.21.77.53l15.63,40.61c.21.54-.19,1.12-.77,1.12h-7.01c-.34,0-.64-.21-.77-.53l-3.22-8.31c-.12-.32-.43-.53-.77-.53h-14.51c-.34,0-.64.21-.77.53l-3.22,8.31c-.12.32-.43.53-.77.53h-7.01c-.58,0-.98-.58-.77-1.12l15.63-40.61c.12-.32.43-.53.77-.53ZM69.93,191.88c.57,0,.97-.57.77-1.11l-5.5-14.95h-.13l-5.33,14.96c-.19.54.21,1.1.78,1.1h9.41ZM130.83,194.33v12.85c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-10.98c0-.21-.08-.41-.22-.56l-3.35-3.55c-.16-.16-.37-.26-.6-.26h-12.68c-.45,0-.82.37-.82.82v14.53c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82h26.74c.21,0,.42.08.57.23l6.31,6.05c.16.16.25.37.25.59v12.1c0,.22-.09.44-.25.59l-3.48,3.35c-.33.31-.34.83-.02,1.16l3.78,3.94c.15.15.23.36.23.57ZM105.34,185.06h14.16c.21,0,.41-.08.57-.23l1.73-1.64c.16-.16.26-.37.26-.6v-7.5c0-.23-.09-.44-.26-.6l-1.73-1.64c-.15-.15-.36-.23-.57-.23h-14.16c-.45,0-.82.37-.82.82v10.79c0,.45.37.82.82.82ZM177.69,165.74h-6.5c-.22,0-.42.14-.49.35l-11.51,29.89c-.1.26-.47.26-.56,0l-11.22-29.9c-.08-.2-.27-.34-.49-.34h-6.49c-.36,0-.62.36-.49.71l15.36,41.02c.12.32.43.53.77.53h6.07c.12,0,.24-.08.28-.19l15.76-41.38c.12-.34-.14-.69-.49-.69ZM189.98,165.74h30.51c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-21.87c-.45,0-.82.37-.82.82v9.04c0,.45.37.82.82.82h20.02c.45,0,.82.37.82.82v5.18c0,.45-.37.82-.82.82h-20.02c-.45,0-.82.37-.82.82v9.22c0,.45.37.82.82.82h21.87c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-30.51c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82ZM245.7,190.53l-14.9-23.53c-.35-.55.05-1.26.7-1.26h7.51c.29,0,.55.15.7.39l10.37,16.93h.13l10.37-16.93c.15-.24.42-.39.7-.39h7.51c.65,0,1.04.72.7,1.26l-14.9,23.53c-.08.13-.13.28-.13.44v16.2c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-16.2c0-.16-.04-.31-.13-.44ZM276.34,172.58v-6.01c0-.45.37-.82.82-.82h7c.45,0,.82.37.82.82v6.01s0,.06,0,.09l-.93,8.04c-.05.42-.4.73-.82.73h-5.14c-.42,0-.77-.31-.82-.73l-.93-8.04s0-.06,0-.09ZM292.06,201.13v-4.86c0-.45.37-.82.82-.82h6.87c.45,0,.82.37.82.82v2.39c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h12.5c.21,0,.41-.08.57-.23l1.67-1.58c.16-.16.26-.37.26-.6v-6.3c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-16.18c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6v-10.46c0-.23.09-.44.26-.6l6.37-6.05c.15-.15.36-.23.57-.23h19.42c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v4.92c0,.45-.37.82-.82.82h-6.87c-.45,0-.82-.37-.82-.82v-2.45c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-11.93c-.21,0-.41.08-.57.23l-1.6,1.52c-.16.16-.26.37-.26.6v5.51c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h16.18c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v11.19c0,.23-.09.44-.26.6l-6.5,6.17c-.15.15-.36.23-.57.23h-19.93c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6ZM163.25,5.54c15.21,0,27.36,5.35,36.13,15.91,9.44,11.37,14.64,28.13,14.64,47.21s-5.2,35.85-14.64,47.21c-8.77,10.56-20.92,15.91-36.13,15.91s-27.36-5.35-36.13-15.91c-9.44-11.37-14.64-28.13-14.64-47.21s5.2-35.85,14.64-47.21c8.77-10.56,20.92-15.91,36.13-15.91M163.25,0c-16.93,0-30.52,6.03-40.39,17.91-10.26,12.36-15.92,30.38-15.92,50.75s5.65,38.39,15.92,50.75c9.87,11.89,23.46,17.91,40.39,17.91s30.52-6.03,40.39-17.91c10.26-12.36,15.92-30.38,15.92-50.75s-5.65-38.39-15.92-50.75C193.78,6.03,180.19,0,163.25,0h0ZM196.56,42.98l-1.97,3.98-.04,50.79,2.01,4.26v4.15h-15.73s-7.61-7.65-7.61-7.65v-23.32s0,0,0,0c-6.21-.91-12.69-1.29-18.68.6-.25.08-.99.28-1.17.38-.03.02-.06.03-.07.07l-.05,29.92h-15.68c-.09-.21-.29-.4-.46-.56-2.2-2.13-4.37-4.31-6.5-6.5-.09-.09-.65-.56-.65-.62v-3.96s1.94-4.23,1.94-4.23l.02-50.53c-.56-1.43-1.3-2.8-1.96-4.19v-4.4h15.73c1.91,2.03,3.97,3.98,5.94,5.97.45.46.98,1.08,1.44,1.52.03.03.23.11.23.12v16.82c6.66-1.04,13.56-.32,19.92,1.82v-26.26s15.73,0,15.73,0c2.35,2.43,4.78,4.83,7.18,7.23.13.13.28.3.44.38v4.19ZM175.86,61.17s0,0,0,0c-2.08-.66-4.12-1.47-6.23-2.04-7.06-1.89-14.53-1.76-21.53.33-1.71.51-3.36,1.18-5.06,1.71v-27.28s-.07-.07-.07-.07h-10.32s0,.98,0,.98c0,.15.03.29.1.43.62,1.29,1.3,2.56,1.82,3.9v51.61c-.53,1.43-1.24,2.8-1.86,4.19-.04.09-.06.19-.06.28h0c0,.38.31.69.69.69h9.63s.07-.07.07-.07v-29.15c1.62-.51,3.22-1.1,4.87-1.54,9.22-2.45,19.1-1.9,27.95,1.61v29.15s10.44,0,10.44,0v-.89c-.68-1.44-1.43-2.85-2.01-4.33l.04-51.93,1.97-4v-.93s-10.44,0-10.44,0v27.35ZM120.44,229.84h15.69v3.64h-11.39v6.14h9.87v3.64h-9.87v8.73h-4.3v-22.14ZM141.82,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM164.16,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM186.49,229.84h14.17l3.8,3.8v14.55l-3.8,3.8h-14.17v-22.14ZM198.54,248.32l1.61-1.61v-11.58l-1.61-1.61h-7.75v14.8h7.75Z"
                      />
                    </g>
                  </svg>
                  <h6 className="text-gray-600 max-w-md text-xl font-bold mb-8 mt-4 h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                    Sub-brand
                  </h6>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Tagline
                  </p>
                  <p className="text-gray-600 max-w-md mb-1">
                    A landmark of the city, a symbol of quality
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Serving Mid-Michigan’s farming and feed needs since 1945
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Brand history
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Since 1866, the Carson City mill has stood next to Fish
                    Creek welcoming residents from near and far with its iconic
                    pitched roof. The vitality of the mill is a testament to the
                    strength and resilience of small towns and small business.
                    What started as a saw mill transitioned to a grist mill and
                    finally the high tech livestock feed mixer it is today,
                    mixing over 30,000 tons annually.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence description
                  </p>
                  <p className="text-gray-600 max-w-md">
                    Harvey's Feed delivers top-quality livestock and specialty
                    feeds—including specializing in custom-blended
                    solutions—backed by expert nutritional support and our
                    partnership with ADM.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-white p-12 flex flex-col items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    id="Layer_2"
                    data-name="Layer 2"
                    viewBox="0 0 326.51 251.99"
                    className="h-40"
                  >
                    <defs />
                    <g id="Layer_1-2" data-name="Layer 1">
                      <path
                        className="cls-1"
                        d="M0,165.74h8.64v17.69h18.81v-17.69h8.64v42.26h-8.64v-17.57H8.64v17.57H0v-42.26ZM61.82,165.74h6.75c.34,0,.65.21.77.53l15.63,40.61c.21.54-.19,1.12-.77,1.12h-7.01c-.34,0-.64-.21-.77-.53l-3.22-8.31c-.12-.32-.43-.53-.77-.53h-14.51c-.34,0-.64.21-.77.53l-3.22,8.31c-.12.32-.43.53-.77.53h-7.01c-.58,0-.98-.58-.77-1.12l15.63-40.61c.12-.32.43-.53.77-.53ZM69.93,191.88c.57,0,.97-.57.77-1.11l-5.5-14.95h-.13l-5.33,14.96c-.19.54.21,1.1.78,1.1h9.41ZM130.83,194.33v12.85c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-10.98c0-.21-.08-.41-.22-.56l-3.35-3.55c-.16-.16-.37-.26-.6-.26h-12.68c-.45,0-.82.37-.82.82v14.53c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82h26.74c.21,0,.42.08.57.23l6.31,6.05c.16.16.25.37.25.59v12.1c0,.22-.09.44-.25.59l-3.48,3.35c-.33.31-.34.83-.02,1.16l3.78,3.94c.15.15.23.36.23.57ZM105.34,185.06h14.16c.21,0,.41-.08.57-.23l1.73-1.64c.16-.16.26-.37.26-.6v-7.5c0-.23-.09-.44-.26-.6l-1.73-1.64c-.15-.15-.36-.23-.57-.23h-14.16c-.45,0-.82.37-.82.82v10.79c0,.45.37.82.82.82ZM177.69,165.74h-6.5c-.22,0-.42.14-.49.35l-11.51,29.89c-.1.26-.47.26-.56,0l-11.22-29.9c-.08-.2-.27-.34-.49-.34h-6.49c-.36,0-.62.36-.49.71l15.36,41.02c.12.32.43.53.77.53h6.07c.12,0,.24-.08.28-.19l15.76-41.38c.12-.34-.14-.69-.49-.69ZM189.98,165.74h30.51c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-21.87c-.45,0-.82.37-.82.82v9.04c0,.45.37.82.82.82h20.02c.45,0,.82.37.82.82v5.18c0,.45-.37.82-.82.82h-20.02c-.45,0-.82.37-.82.82v9.22c0,.45.37.82.82.82h21.87c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-30.51c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82ZM245.7,190.53l-14.9-23.53c-.35-.55.05-1.26.7-1.26h7.51c.29,0,.55.15.7.39l10.37,16.93h.13l10.37-16.93c.15-.24.42-.39.7-.39h7.51c.65,0,1.04.72.7,1.26l-14.9,23.53c-.08.13-.13.28-.13.44v16.2c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-16.2c0-.16-.04-.31-.13-.44ZM276.34,172.58v-6.01c0-.45.37-.82.82-.82h7c.45,0,.82.37.82.82v6.01s0,.06,0,.09l-.93,8.04c-.05.42-.4.73-.82.73h-5.14c-.42,0-.77-.31-.82-.73l-.93-8.04s0-.06,0-.09ZM292.06,201.13v-4.86c0-.45.37-.82.82-.82h6.87c.45,0,.82.37.82.82v2.39c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h12.5c.21,0,.41-.08.57-.23l1.67-1.58c.16-.16.26-.37.26-.6v-6.3c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-16.18c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6v-10.46c0-.23.09-.44.26-.6l6.37-6.05c.15-.15.36-.23.57-.23h19.42c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v4.92c0,.45-.37.82-.82.82h-6.87c-.45,0-.82-.37-.82-.82v-2.45c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-11.93c-.21,0-.41.08-.57.23l-1.6,1.52c-.16.16-.26.37-.26.6v5.51c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h16.18c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v11.19c0,.23-.09.44-.26.6l-6.5,6.17c-.15.15-.36.23-.57.23h-19.93c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6ZM163.25,5.54c15.21,0,27.36,5.35,36.13,15.91,9.44,11.37,14.64,28.13,14.64,47.21s-5.2,35.85-14.64,47.21c-8.77,10.56-20.92,15.91-36.13,15.91s-27.36-5.35-36.13-15.91c-9.44-11.37-14.64-28.13-14.64-47.21s5.2-35.85,14.64-47.21c8.77-10.56,20.92-15.91,36.13-15.91M163.25,0c-16.93,0-30.52,6.03-40.39,17.91-10.26,12.36-15.92,30.38-15.92,50.75s5.65,38.39,15.92,50.75c9.87,11.89,23.46,17.91,40.39,17.91s30.52-6.03,40.39-17.91c10.26-12.36,15.92-30.38,15.92-50.75s-5.65-38.39-15.92-50.75C193.78,6.03,180.19,0,163.25,0h0ZM196.56,42.98l-1.97,3.98-.04,50.79,2.01,4.26v4.15h-15.73s-7.61-7.65-7.61-7.65v-23.32s0,0,0,0c-6.21-.91-12.69-1.29-18.68.6-.25.08-.99.28-1.17.38-.03.02-.06.03-.07.07l-.05,29.92h-15.68c-.09-.21-.29-.4-.46-.56-2.2-2.13-4.37-4.31-6.5-6.5-.09-.09-.65-.56-.65-.62v-3.96s1.94-4.23,1.94-4.23l.02-50.53c-.56-1.43-1.3-2.8-1.96-4.19v-4.4h15.73c1.91,2.03,3.97,3.98,5.94,5.97.45.46.98,1.08,1.44,1.52.03.03.23.11.23.12v16.82c6.66-1.04,13.56-.32,19.92,1.82v-26.26s15.73,0,15.73,0c2.35,2.43,4.78,4.83,7.18,7.23.13.13.28.3.44.38v4.19ZM175.86,61.17s0,0,0,0c-2.08-.66-4.12-1.47-6.23-2.04-7.06-1.89-14.53-1.76-21.53.33-1.71.51-3.36,1.18-5.06,1.71v-27.28s-.07-.07-.07-.07h-10.32s0,.98,0,.98c0,.15.03.29.1.43.62,1.29,1.3,2.56,1.82,3.9v51.61c-.53,1.43-1.24,2.8-1.86,4.19-.04.09-.06.19-.06.28h0c0,.38.31.69.69.69h9.63s.07-.07.07-.07v-29.15c1.62-.51,3.22-1.1,4.87-1.54,9.22-2.45,19.1-1.9,27.95,1.61v29.15s10.44,0,10.44,0v-.89c-.68-1.44-1.43-2.85-2.01-4.33l.04-51.93,1.97-4v-.93s-10.44,0-10.44,0v27.35ZM18.41,248.35v-14.87l3.64-3.64h11.01l3.57,3.57v3.38h-4.3v-1.8l-1.49-1.49h-6.52l-1.61,1.61v11.58l1.61,1.61h6.58l1.55-1.55v-3.45h-4.75v-3.67h8.92v8.7l-3.64,3.64h-10.94l-3.64-3.64ZM49.16,229.84h3.92l8.1,22.14h-4.37l-1.8-4.9h-7.78l-1.8,4.9h-4.37l8.1-22.14ZM54.06,243.54l-2.94-8.42h-.06l-2.85,8.42h5.85ZM83.95,244.65v7.34h-4.3v-6.36l-1.9-2.12h-6.9v8.48h-4.3v-22.14h13.89l3.38,3.42v6.71l-2.15,2.18,2.28,2.5ZM70.86,239.97h7.62l1.11-1.11v-4.3l-1.11-1.11h-7.62v6.52ZM91.07,229.84h14.17l3.8,3.8v14.55l-3.8,3.8h-14.17v-22.14ZM103.12,248.32l1.61-1.61v-11.58l-1.61-1.61h-7.75v14.8h7.75ZM116,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM138.33,229.84h3.86l9.87,15.03h.06v-15.03h4.14v22.14h-3.86l-9.87-14.99h-.06v14.99h-4.14v-22.14ZM174.99,248.35v-14.87l3.64-3.64h10.82l3.57,3.57v3.38h-4.3v-1.8l-1.49-1.49h-6.33l-1.61,1.61v11.58l1.61,1.61h6.33l1.49-1.49v-1.8h4.3v3.38l-3.57,3.57h-10.82l-3.64-3.64ZM199.19,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM221.52,229.84h3.86l9.87,15.03h.06v-15.03h4.14v22.14h-3.86l-9.87-14.99h-.06v14.99h-4.14v-22.14ZM251.76,233.48h-6.61v-3.64h17.52v3.64h-6.61v18.5h-4.3v-18.5ZM268.37,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM308.1,244.65v7.34h-4.3v-6.36l-1.9-2.12h-6.9v8.48h-4.3v-22.14h13.89l3.38,3.42v6.71l-2.15,2.18,2.28,2.5ZM295,239.97h7.62l1.11-1.11v-4.3l-1.11-1.11h-7.62v6.52Z"
                      />
                    </g>
                  </svg>
                  <h6 className="text-gray-600 max-w-md text-xl font-bold mb-8 mt-4 h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                    Sub-brand
                  </h6>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Brand history
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    I don't have a clue about the history of the GC, to be
                    honest.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence description
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Harvey’s seasonal Garden Center offers a wide selection of
                    annuals, perennials, trees, shrubs, and gardening essentials
                    to bring landscape to life.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-SENTENCE CTA
                  </p>
                  <p className="text-gray-600 max-w-md">
                    Located right next door to Harvey’s, our seasonal Garden
                    Center is your go-to spot for beautiful annuals, perennials,
                    trees, shrubs, and everything else you need to make your
                    yard shine.
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/2 p-4">
                <div className="bg-white p-12 flex flex-col items-center">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    id="Layer_2"
                    data-name="Layer 2"
                    viewBox="0 0 326.51 251.99"
                    className="h-40"
                  >
                    <defs />
                    <g id="Layer_1-2" data-name="Layer 1">
                      <path
                        className="cls-1"
                        d="M0,165.74h8.64v17.69h18.81v-17.69h8.64v42.26h-8.64v-17.57H8.64v17.57H0v-42.26ZM61.82,165.74h6.75c.34,0,.65.21.77.53l15.63,40.61c.21.54-.19,1.12-.77,1.12h-7.01c-.34,0-.64-.21-.77-.53l-3.22-8.31c-.12-.32-.43-.53-.77-.53h-14.51c-.34,0-.64.21-.77.53l-3.22,8.31c-.12.32-.43.53-.77.53h-7.01c-.58,0-.98-.58-.77-1.12l15.63-40.61c.12-.32.43-.53.77-.53ZM69.93,191.88c.57,0,.97-.57.77-1.11l-5.5-14.95h-.13l-5.33,14.96c-.19.54.21,1.1.78,1.1h9.41ZM130.83,194.33v12.85c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-10.98c0-.21-.08-.41-.22-.56l-3.35-3.55c-.16-.16-.37-.26-.6-.26h-12.68c-.45,0-.82.37-.82.82v14.53c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82h26.74c.21,0,.42.08.57.23l6.31,6.05c.16.16.25.37.25.59v12.1c0,.22-.09.44-.25.59l-3.48,3.35c-.33.31-.34.83-.02,1.16l3.78,3.94c.15.15.23.36.23.57ZM105.34,185.06h14.16c.21,0,.41-.08.57-.23l1.73-1.64c.16-.16.26-.37.26-.6v-7.5c0-.23-.09-.44-.26-.6l-1.73-1.64c-.15-.15-.36-.23-.57-.23h-14.16c-.45,0-.82.37-.82.82v10.79c0,.45.37.82.82.82ZM177.69,165.74h-6.5c-.22,0-.42.14-.49.35l-11.51,29.89c-.1.26-.47.26-.56,0l-11.22-29.9c-.08-.2-.27-.34-.49-.34h-6.49c-.36,0-.62.36-.49.71l15.36,41.02c.12.32.43.53.77.53h6.07c.12,0,.24-.08.28-.19l15.76-41.38c.12-.34-.14-.69-.49-.69ZM189.98,165.74h30.51c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-21.87c-.45,0-.82.37-.82.82v9.04c0,.45.37.82.82.82h20.02c.45,0,.82.37.82.82v5.18c0,.45-.37.82-.82.82h-20.02c-.45,0-.82.37-.82.82v9.22c0,.45.37.82.82.82h21.87c.45,0,.82.37.82.82v5.3c0,.45-.37.82-.82.82h-30.51c-.45,0-.82-.37-.82-.82v-40.61c0-.45.37-.82.82-.82ZM245.7,190.53l-14.9-23.53c-.35-.55.05-1.26.7-1.26h7.51c.29,0,.55.15.7.39l10.37,16.93h.13l10.37-16.93c.15-.24.42-.39.7-.39h7.51c.65,0,1.04.72.7,1.26l-14.9,23.53c-.08.13-.13.28-.13.44v16.2c0,.45-.37.82-.82.82h-7c-.45,0-.82-.37-.82-.82v-16.2c0-.16-.04-.31-.13-.44ZM276.34,172.58v-6.01c0-.45.37-.82.82-.82h7c.45,0,.82.37.82.82v6.01s0,.06,0,.09l-.93,8.04c-.05.42-.4.73-.82.73h-5.14c-.42,0-.77-.31-.82-.73l-.93-8.04s0-.06,0-.09ZM292.06,201.13v-4.86c0-.45.37-.82.82-.82h6.87c.45,0,.82.37.82.82v2.39c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h12.5c.21,0,.41-.08.57-.23l1.67-1.58c.16-.16.26-.37.26-.6v-6.3c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-16.18c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6v-10.46c0-.23.09-.44.26-.6l6.37-6.05c.15-.15.36-.23.57-.23h19.42c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v4.92c0,.45-.37.82-.82.82h-6.87c-.45,0-.82-.37-.82-.82v-2.45c0-.23-.09-.44-.26-.6l-1.6-1.52c-.15-.15-.36-.23-.57-.23h-11.93c-.21,0-.41.08-.57.23l-1.6,1.52c-.16.16-.26.37-.26.6v5.51c0,.23.09.44.26.6l1.6,1.52c.15.15.36.23.57.23h16.18c.21,0,.41.08.57.23l6.37,6.05c.16.16.26.37.26.6v11.19c0,.23-.09.44-.26.6l-6.5,6.17c-.15.15-.36.23-.57.23h-19.93c-.21,0-.41-.08-.57-.23l-6.37-6.05c-.16-.16-.26-.37-.26-.6ZM163.25,5.54c15.21,0,27.36,5.35,36.13,15.91,9.44,11.37,14.64,28.13,14.64,47.21s-5.2,35.85-14.64,47.21c-8.77,10.56-20.92,15.91-36.13,15.91s-27.36-5.35-36.13-15.91c-9.44-11.37-14.64-28.13-14.64-47.21s5.2-35.85,14.64-47.21c8.77-10.56,20.92-15.91,36.13-15.91M163.25,0c-16.93,0-30.52,6.03-40.39,17.91-10.26,12.36-15.92,30.38-15.92,50.75s5.65,38.39,15.92,50.75c9.87,11.89,23.46,17.91,40.39,17.91s30.52-6.03,40.39-17.91c10.26-12.36,15.92-30.38,15.92-50.75s-5.65-38.39-15.92-50.75C193.78,6.03,180.19,0,163.25,0h0ZM196.56,42.98l-1.97,3.98-.04,50.79,2.01,4.26v4.15h-15.73s-7.61-7.65-7.61-7.65v-23.32s0,0,0,0c-6.21-.91-12.69-1.29-18.68.6-.25.08-.99.28-1.17.38-.03.02-.06.03-.07.07l-.05,29.92h-15.68c-.09-.21-.29-.4-.46-.56-2.2-2.13-4.37-4.31-6.5-6.5-.09-.09-.65-.56-.65-.62v-3.96s1.94-4.23,1.94-4.23l.02-50.53c-.56-1.43-1.3-2.8-1.96-4.19v-4.4h15.73c1.91,2.03,3.97,3.98,5.94,5.97.45.46.98,1.08,1.44,1.52.03.03.23.11.23.12v16.82c6.66-1.04,13.56-.32,19.92,1.82v-26.26s15.73,0,15.73,0c2.35,2.43,4.78,4.83,7.18,7.23.13.13.28.3.44.38v4.19ZM175.86,61.17s0,0,0,0c-2.08-.66-4.12-1.47-6.23-2.04-7.06-1.89-14.53-1.76-21.53.33-1.71.51-3.36,1.18-5.06,1.71v-27.28s-.07-.07-.07-.07h-10.32s0,.98,0,.98c0,.15.03.29.1.43.62,1.29,1.3,2.56,1.82,3.9v51.61c-.53,1.43-1.24,2.8-1.86,4.19-.04.09-.06.19-.06.28h0c0,.38.31.69.69.69h9.63s.07-.07.07-.07v-29.15c1.62-.51,3.22-1.1,4.87-1.54,9.22-2.45,19.1-1.9,27.95,1.61v29.15s10.44,0,10.44,0v-.89c-.68-1.44-1.43-2.85-2.01-4.33l.04-51.93,1.97-4v-.93s-10.44,0-10.44,0v27.35ZM122.4,244.65v7.34h-4.3v-6.36l-1.9-2.12h-6.9v8.48h-4.3v-22.14h13.89l3.38,3.42v6.71l-2.15,2.18,2.28,2.5ZM109.31,239.97h7.62l1.11-1.11v-4.3l-1.11-1.11h-7.62v6.52ZM129.52,229.84h16.01v3.64h-11.7v5.6h10.79v3.58h-10.79v5.69h11.7v3.64h-16.01v-22.14ZM156.88,233.48h-6.61v-3.64h17.52v3.64h-6.61v18.5h-4.3v-18.5ZM176.84,229.84h3.92l8.1,22.14h-4.37l-1.8-4.9h-7.78l-1.8,4.9h-4.37l8.1-22.14ZM181.74,243.54l-2.94-8.42h-.06l-2.85,8.42h5.85ZM194.4,229.84h4.3v22.14h-4.3v-22.14ZM206.13,229.84h4.3v18.5h11.07v3.64h-15.37v-22.14Z"
                      />
                    </g>
                  </svg>
                  <h6 className="text-gray-600 max-w-md text-xl font-bold mb-8 mt-4 h6 text-xl font-bold mb-4 font-heading uppercase tracking-widest">
                    Sub-brand
                  </h6>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Tagline
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    Your go-to place for feed, seed, and clothing needsf
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    Brand history
                  </p>
                  <p className="text-gray-600 max-w-md mb-8">
                    The original outpost for Montcalm County when a gristmill
                    was built along Fish Creek in 1870, we're not sure when the
                    general store first opened, but we try to bring a little of
                    that frontier charm to Harvey's.
                  </p>
                  <p className="gray-accent text-gray-400 font-bold mb-4 font-heading uppercase tracking-widest">
                    1-sentence description
                  </p>
                  <p className="text-gray-600 max-w-md">
                    Harvey's is Mid-Michigan's premier destination for
                    exceptional feed, seed, apparel, and footwear, delivering
                    unmatched quality, expert guidance, and a heritage of
                    trusted service since 1946.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </section>
      </>
    </React.Fragment>
  );
}
