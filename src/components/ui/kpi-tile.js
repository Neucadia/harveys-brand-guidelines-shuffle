...
const KpiTile = React.forwardRef(
  ({ label, value, sub, negative = false, className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("border border-gray-200 bg-white px-4 py-3", className)}
      {...props}
    >
      <div className="font-heading text-xs font-bold uppercase tracking-smallcaps text-gray-500">
        {label}
      </div>
      <div
        className="mt-1 font-heading text-2xl font-bold tabular-nums text-black"
